...
const { getMonitoringUserByID, getUptimePlatform } = require("./helper/sql-monitoring.js");
const { STAGE } = process.env;

class GetUptimePlatform extends BaseHandler {
    constructor() {
        super();
    }
    ...
    async process(event, context, callback) {
        try {
            ...
            let results = await getUptimePlatform(dbHelper, platform_id);
            dbHelper.conn.end();
            if (results) {
                let grouped_result = {};
                let output = {};
                ...
                for (const group_level_key in grouped_result) {
                    ...
                    for (const resource_level_key in grouped_result[group_level_key]) {
                        ...
                        const resultbydate = helper.persentDays(arrayOfObjects);
                ...
                return responseHandler.sendBadReqResponse({message: 'Uptime Platform could not be fetched.'});
            }
        } catch(e) {
...
exports.get_uptime_platform = async(event, context, callback) => {
    return await new GetUptimePlatform().handler(event, context, callback);
};