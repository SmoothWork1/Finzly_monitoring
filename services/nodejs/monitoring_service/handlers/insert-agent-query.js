...
const { getMonitoringUserByID, insertAgentQuery } = require('./helper/sql-monitoring.js');
const { STAGE } = process.env;

class Insert_Agent_Query extends BaseHandler {
    ...
    async process(event, context, callback) {
        try {
            ...
            const agent_query = await insertAgentQuery(dbHelper, {
                query: body.query,
            ...
            dbHelper.conn.end();
            this.log.info("Agent query added: ", agent_query);
            if (agent_query) {
                let resp = {
                    message: "Agent query added successfully"
                };
                return responseHandler.sendSuccessResponse(resp);
            } else {
                return responseHandler.sendBadReqResponse({message: 'Agent query could not be added.'});
            }
        } catch(e) {
            this.log.error("Add agent query Error: ", e);
            return responseHandler.sendBadReqResponse({message: 'Could not add agent query.'});
        }
    }
}

exports.insert_agent_query = async(event, context, callback) => {
    return await new Insert_Agent_Query().handler(event, context, callback);
};